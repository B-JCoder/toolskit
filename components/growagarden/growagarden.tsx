...
const PLANTS = [
  { name: "Tomatoes", space: 24, type: "Vegetable", note: "Needs 2 feet of space" },
  { name: "Carrots", space: 3, type: "Vegetable", note: "Can be planted densely" },
  { name: "Lettuce", space: 6, type: "Vegetable", note: "Good for small spaces" },
  { name: "Peppers", space: 18, type: "Vegetable", note: "Space 18 inches apart" },
  { name: "Beans (Bush)", space: 4, type: "Vegetable", note: "4-6 inches apart" },
  { name: "Cucumbers", space: 12, type: "Vegetable", note: "Needs trellis or space" },
]
    ...
    <Card className="max-w-4xl mx-auto p-6 space-y-8">
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
            ...
            <div className="p-4 bg-muted/50 rounded-lg">
                ...
                <div className="text-2xl font-bold text-primary">{totalArea} sq. ft.</div>
            </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="p-4 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900 rounded-lg">
                <div className="flex justify-between items-start mb-2">
                    <span className="font-semibold text-green-900 dark:text-green-300">{plantInfo.name}</span>
                    <div className="text-xs bg-white/50 px-2 py-1 rounded">{plantInfo.type}</div>
                </div>
      ...
      <div className="border-t pt-8">
        <h3 className="text-2xl font-bold text-center mb-8">
            You can grow approximately <span className="text-primary">{totalPlants}</span> {selectedPlant} plants!
        </h3>
        ...
        <div className="w-full h-64 bg-[#3e2723] rounded-lg p-2 relative overflow-hidden flex flex-wrap content-start gap-1 justify-center shadow-inner">
            {Array.from({ length: Math.min(totalPlants, 100) }).map((_, i) => (
                <div key={i} className="w-4 h-4 bg-green-500 rounded-full shadow-sm animate-in zoom-in duration-300" style={{ animationDelay: `${i * 10}ms` }} />
            ))}
            {totalPlants > 100 && (
