        ...
        <div className="grid gap-8">
          <Card className="bg-card shadow-lg border-border/50 rounded-2xl">
            ...
            <CardContent>
              <div className="flex items-center gap-4 flex-wrap">
                ...
                />
                <span className="text-sm text-muted-foreground ml-auto">{duration}s test</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card shadow-lg border-border/50 rounded-2xl">
            <CardHeader>
              ...
              <CardDescription>{isActive ? 'Click as fast as you can!' : 'Click the start button to begin'}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-center space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div className="p-4 rounded-lg bg-primary/10">
                    <div className="text-4xl font-bold text-primary">{clicks}</div>
                    <div className="text-sm text-muted-foreground mt-1">Clicks</div>
                  </div>
                  <div className="p-4 rounded-lg bg-primary/10">
                    <div className="text-4xl font-bold text-primary">{displayValues.displayTime}</div>
                    <div className="text-sm text-muted-foreground mt-1">{displayValues.timeLabel}</div>
                  </div>
                  <div className="p-4 rounded-lg bg-primary/10">
                    <div className="text-4xl font-bold text-primary">{displayValues.formattedCurrentCps}</div>
                    <div className="text-sm text-muted-foreground mt-1">Current CPS</div>
                  </div>
                </div>

                {/* Action Section */}
                {testNotStarted ? (
                  <Button onClick={startTest} size="lg" className="w-full sm:w-auto px-8 py-6 text-lg font-semibold">
                  ...
                  <button
                    onClick={handleClick}
                    className="w-full h-48 sm:h-40 bg-gradient-to-br from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 border-2 border-primary/30 rounded-lg cursor-pointer flex items-center justify-center transition-all active:scale-95 select-none font-bold text-xl text-white shadow-lg"
                  >
                    Click Here!
                  </button>
                ) : (
                  <div className="space-y-4">
                    <div className="text-lg font-semibold text-foreground">Test completed!</div>
              ...
              <CardContent>
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div className="text-center">
                    <div className="text-5xl font-bold text-yellow-500">{displayValues.formattedBestCps}</div>
                    <div className="text-sm text-muted-foreground mt-2">Best CPS</div>
                  </div>
                  <Button onClick={resetBest} variant="outline" size="sm" className="border-yellow-300 hover:bg-yellow-50 dark:hover:bg-yellow-950/20">
                    Reset Best
                  </Button>
                </div>
              </CardContent>
