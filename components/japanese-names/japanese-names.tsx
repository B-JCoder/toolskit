...
const JapaneseNamesComponent = () => {
  ...
  const buttonLabel = useMemo(() => (generatedName ? 'Generate New Name' : 'Generate Name'), [generatedName])
      ...
      <div className="space-y-8">
        ...
        {generatedName && (
          <div className="p-8 rounded-xl bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20 text-center space-y-4">
            <div className="text-4xl font-bold text-foreground">{generatedName}</div>
            <Button
        ...
        <div className="text-center">
          <Button onClick={generateName} size="lg" className="gap-2 px-8 py-6 text-lg font-semibold">
            <RefreshCw className={`h-5 w-5 ${generatedName ? 'group-hover:rotate-180' : ''}`} />
            {buttonLabel}
          </Button>
        </div>
        ...
        <div className="p-4 bg-muted/50 rounded-lg text-center text-sm text-muted-foreground space-y-2">
          <p>✨ Names are generated using authentic Japanese naming conventions</p>
          <p>🎨 Perfect for characters, usernames, or creative projects</p>
          <p className="text-xs mt-3">
            Each name combines a traditional first name with a common Japanese surname
          </p>
        </div>
      </div>
    </ToolCard>
